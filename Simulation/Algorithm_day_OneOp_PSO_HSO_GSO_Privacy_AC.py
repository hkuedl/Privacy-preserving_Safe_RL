...
class BatchRLAlgorithm:
    def __init__(self, trainer, env, policy, replay_buffer, num_epochs, num_env_steps_per_epoch, num_trains_per_train_epoch, min_num_epoch_before_training, batch_size, **env_args):
        self.env = env
    ...
    def train(self):
        ...
        for self.epoch in tqdm(range(self.max_epoch_num)):
            ...
            if len(reward_curve) % 100 == 0 and len(reward_curve) != 0:
                ...
                plt.grid()
                plt.savefig('workspaces/Multi_energy/Results/R/Reward.png')
                plt.show()
                plt.close()
                ...
                plt.grid()
                plt.savefig('workspaces/Multi_energy/Results/R/Cost.png')
                plt.show()
                plt.close()

            if len(reward_curve) % 500 == 0 and len(reward_curve) != 0:
                np.save('workspaces/Multi_energy/Results/R/Cost.npy', np.array(reward_curve))
            end_time = time.time()
        ...
        np.save('workspaces/Multi_energy/Results/R/his_obs.npy', np.array(HO))
        np.save('workspaces/Multi_energy/Results/R/his_obs_encry.npy', np.array(HO_en))
        np.save('workspaces/Multi_energy/Results/R/his_sac.npy', np.array(HSAC))
        np.save('workspaces/Multi_energy/Results/R/his_action.npy', np.array(HA))
        np.save('workspaces/Multi_energy/Results/R/his_trade.npy', np.array(T))
        np.save('workspaces/Multi_energy/Results/R/his_OV.npy', np.array(OV))
        np.save('workspaces/Multi_energy/Results/R/Reward.npy', np.array(reward_curve))
        np.save('workspaces/Multi_energy/Results/R/Cost.npy', np.array(cost_curve))
        
        # 保存policy模型的状态字典
        torch.save(self.policy.state_dict(), 'workspaces/Multi_energy/Results/R/policy.pth')
