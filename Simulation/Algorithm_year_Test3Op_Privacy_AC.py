...
from Simulation.Correction_Module_PSO_HSO_GSOV2 import Correction_Module
...
class BatchRLAlgorithm:
    ...
    def test(self):
        ...
        np.save('workspaces/Multi_energy/Results/Test/his_obs.npy', np.array(HO))
        np.save('workspaces/Multi_energy/Results/Test/his_obs_encry.npy', np.array(HO_en))
        np.save('workspaces/Multi_energy/Results/Test/his_sac.npy', np.array(HSAC))
        np.save('workspaces/Multi_energy/Results/Test/his_action.npy', np.array(HA))
        np.save('workspaces/Multi_energy/Results/Test/his_trade.npy', np.array(T))
        np.save('workspaces/Multi_energy/Results/Test/his_OV.npy', np.array(OV))
        np.save('workspaces/Multi_energy/Results/Test/Reward.npy', np.array(reward_curve))
        np.save('workspaces/Multi_energy/Results/Test/Cost.npy', np.array(cost_curve))
        np.save('workspaces/Multi_energy/Results/Test/pso_flow_p.npy', np.array(PSO_P))
        np.save('workspaces/Multi_energy/Results/Test/pso_flow_q.npy', np.array(PSO_Q))
        np.save('workspaces/Multi_energy/Results/Test/Voltage.npy', np.array(VOL))
        np.save('workspaces/Multi_energy/Results/Test/gso_flow.npy', np.array(GSO_F))
        np.save('workspaces/Multi_energy/Results/Test/Pressure.npy', np.array(PRES))
        np.save('workspaces/Multi_energy/Results/Test/temp_supply.npy', np.array(TEMP_S))
        np.save('workspaces/Multi_energy/Results/Test/temp_return.npy', np.array(TEMP_R))
    ...
    def run_sim(self, epoch, obs=None, obs_encry=None):
        ...
        for env_step in range(self.max_env_step):
            ...
            len = self.env.get_action_dim() + 1
            a = np.zeros(len)                  

            # PSO & GSO action
            for action, i in zip(a_sac, range(8)):       #除了储能外，其他action的范围为[0,1]，而policy输出的action为[-1,1],因此需要处理一下再输出。
                if i == 4 :
                    a[i] = action
                elif i == 7:
                    a[i] = action
            ...
            # HSO action                    (a_asc[6]给出的是EHP和GB分别承担的负荷比例，a_sac[7]给出的是TS的动作)
            a[8] = (a_sac[8] + 1)/2
            a[9] = 1 - a[8]
            a[10] = a_sac[9]
